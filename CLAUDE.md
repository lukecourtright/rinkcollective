...
**`adminOnly`** (`Optional[bool]`, omitted from the schema above since almost no rink sets it — `null`/absent is treated the same as `false`) — hides a rink from `GET /api/rinks` for everyone except a signed-in `ADMIN_EMAILS` user; `main.py`'s `get_rinks()` checks the session and filters the query result when the requester isn't an admin. Deliberately nullable rather than a plain `bool` default `False`: `ensure_new_columns()` (see Backend above) adds new columns via a raw `ALTER TABLE ... ADD COLUMN`, and Postgres rejects `NOT NULL` on that against a table that already has rows unless the column also has a server-side `DEFAULT` — a `bool = False` SQLModel field only gets a *Python-level* default, not one visible to the DDL, so the first deploy of this field crashed production startup (`NotNullViolation`) until it was changed to nullable. Keep this in mind for any future non-`Optional` field added to an existing table — either make it `Optional` (as here), or reach for a proper `server_default` (nothing does that yet in this codebase). Meant for admin-side scratch/test rinks (e.g. id 660, `"ADMIN TEST — Do Not Publish"`, added purely so an admin can exercise check-ins/reviews/photos/Owner Console features without a real rink in the way) rather than anything content-related — there's no Admin Console UI to toggle it yet, it's hand-set in `rinks.json`. Nav-only, same caveat as the Equipment link below: rink-scoped endpoints (`/api/rinks/{id}/schedule`, `/photos`, etc.) aren't filtered by it, so this isn't a hard access-control boundary, just keeps it off the public list/map.
...
**The "Equipment" nav link is admin-only for now** — hidden (not just unlinked) from the top nav and mobile menu on `index.html`/`home.html`/`guides.html` unless the signed-in user's `isAdmin` flag is set, since pricing/retailers are still mock data (see Live Offers below) and a public link would confuse visitors before it's real. Each page's own account-nav render function (`renderAccountNav()`/the equivalent block in `index.html`'s `render()`) toggles `#nav-link-equipment`/`#menu-link-equipment` alongside the existing admin-only elements (e.g. `account-menu-admin`) — same `isAdmin`-gated pattern, just applied to a nav link instead of a menu item. This is nav-only, not a hard gate: `/equipment` itself isn't blocked server-side, so a direct link still works for anyone who has one. `equipment.html`'s own nav (which links to itself) is unchanged. Revisit once Live Offers has enough real inventory to make the page public again — just drop the `isAdmin` check on those two elements.
...
- Real schema migrations (no Alembic) — `ensure_new_columns()` in `main.py` covers the common case of adding a new nullable column, but column renames/type changes/drops still have no automated path and would need a manual `ALTER TABLE` against the Railway Postgres addon. **A non-nullable column crashes production startup** — `ensure_new_columns()`'s `ALTER TABLE ... ADD COLUMN` has no `server_default`, so Postgres rejects `NOT NULL` against a table that already has rows (a plain SQLModel `= False`/`= 0` default is Python-only, invisible to the DDL); this actually happened once, see `Rink.adminOnly` above for the incident and the nullable-field fix. Any new field on an existing table must be `Optional` (or ship a real `server_default`, which nothing in this codebase does yet) — SQLite-backed local dev won't catch this because a fresh `dev.db` creates the column via `CREATE TABLE`, not `ALTER TABLE`; test against a `dev.db` that already has the table *without* the new column to catch it before deploying
