...
**`adminOnly`** (bool, default `false`, omitted from the schema above since almost no rink sets it) — hides a rink from `GET /api/rinks` for everyone except a signed-in `ADMIN_EMAILS` user; `main.py`'s `get_rinks()` checks the session and filters the query result when the requester isn't an admin. Meant for admin-side scratch/test rinks (e.g. id 660, `"ADMIN TEST — Do Not Publish"`, added purely so an admin can exercise check-ins/reviews/photos/Owner Console features without a real rink in the way) rather than anything content-related — there's no Admin Console UI to toggle it yet, it's hand-set in `rinks.json`. Nav-only, same caveat as the Equipment link below: rink-scoped endpoints (`/api/rinks/{id}/schedule`, `/photos`, etc.) aren't filtered by it, so this isn't a hard access-control boundary, just keeps it off the public list/map.
